...
export function UsersView({ users, sessions, userStats, darkMode }: UsersViewProps) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
      ...
      <div className="lg:col-span-3 flex flex-col gap-3">
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm p-4 overflow-hidden flex flex-col gap-3">
          <div className="flex items-center justify-between">
            <h3 className="text-[11px] font-bold text-slate-900 dark:text-white uppercase tracking-tight">Active Accounts</h3>
            <span className="bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 px-1.5 py-0.5 rounded text-[9px] font-bold uppercase tracking-widest">{users.length} Total</span>
          </div>
          ...
          <div className="relative">
            ...
            <input 
              ...
              className="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-lg py-1.5 pl-9 pr-3 text-[11px] font-medium focus:ring-1 focus:ring-indigo-100 dark:focus:ring-indigo-900/40 text-slate-900 dark:text-white placeholder:text-slate-400"
            />
          </div>
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    isActive ? "bg-indigo-600 text-white shadow-md shadow-indigo-100 dark:shadow-none" : "hover:bg-slate-50 dark:hover:bg-slate-800"
                  )}
                >
                  <div className="flex items-center gap-2 text-left">
                    <div className={cn(
                      "w-8 h-8 rounded-full flex items-center justify-center border",
                      isActive ? "bg-white/20 border-white/20" : "bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-800 shadow-sm"
                    )}>
                      <UserIcon size={14} className={isActive ? "text-white" : "text-slate-600 dark:text-slate-400"} />
                    </div>
                    <div>
                      <p className={cn("text-[11px] font-bold uppercase tracking-tight", isActive ? "text-white" : "text-slate-900 dark:text-white")}>
                        {user.username}
                      </p>
                  ...
                  {!isActive && stats && (
                    <div className="text-right">
                      <p className="text-[9px] font-bold text-slate-900 dark:text-white">{formatDuration(stats.totalTime)}</p>
      ...
      <div className="lg:col-span-9">
        {!selectedUserId ? (
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 border-dashed h-full min-h-[400px] flex flex-col items-center justify-center text-slate-400 gap-3 p-8">
            <div className="w-12 h-12 bg-slate-50 dark:bg-slate-800 rounded-full flex items-center justify-center opacity-50">
              <Users size={24} />
            </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm p-6 relative overflow-hidden">
               <div className="flex flex-col md:flex-row gap-6 items-start md:items-center relative z-10">
                <div className="w-16 h-16 rounded-2xl bg-indigo-600 flex items-center justify-center text-white shadow-lg shadow-indigo-100 dark:shadow-none border-2 border-white dark:border-slate-800">
                  <UserIcon size={32} />
                </div>
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <h2 className="text-lg font-bold text-slate-900 dark:text-white tracking-tight">{selectedUser?.username}</h2>
                    <span className="px-1.5 py-0.5 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 rounded text-[9px] font-bold uppercase tracking-widest">
                      {selectedUser?.type}
                    </span>
                  </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm p-4">
                <h3 className="text-[11px] font-bold text-slate-900 dark:text-white uppercase tracking-tight mb-4">Recent Activity (14 Days)</h3>
                <div className="h-[180px] w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={userActivityChartData}>
                      ...
                      />
                      <Bar dataKey="hours" fill={darkMode ? "#7184bc" : "#6366f1"} radius={[2, 2, 0, 0]} barSize={16} />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              </div>

              <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm p-4 flex flex-col gap-2">
                <h3 className="text-[11px] font-bold text-slate-900 dark:text-white uppercase tracking-tight mb-2">Listener Profile</h3>
                {[
                  ...
                  <div key={item.label} className="flex items-center justify-between p-2 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-50 dark:border-slate-800">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden mb-6">
              <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
                <div>
                  <h3 className="text-[11px] font-bold text-slate-900 dark:text-white uppercase tracking-tight">Listening Sessions</h3>
                  <p className="text-[9px] text-slate-500 uppercase tracking-widest font-semibold mt-0.5">Detailed playback history</p>
                </div>
                <button className="flex items-center gap-2 px-2 py-1 bg-slate-50 dark:bg-slate-800 rounded-md text-[9px] font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors">
                  ...
                  <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                    {selectedUserSessions.map((session) => (
                      <tr key={session.id} className="group hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors">
                        ...
                        <td className="px-4 py-2">
                          <p className="text-[11px] font-bold text-slate-900 dark:text-white">{session.displayTitle || session.mediaItemTitle}</p>
